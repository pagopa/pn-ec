...
//      Insert request from SMS request and publish to Notification Tracker with next status -> BOOKED
		return insertRequestFromSms(digitalCourtesySmsRequest, xPagopaExtchCxId).then(sqsService.send(
				notificationTrackerSqsName.statoSmsName(),
				new NotificationTrackerQueueDto(presaInCaricoInfo.getRequestIdx(),
						presaInCaricoInfo.getXPagopaExtchCxId(), now(), transactionProcessConfigurationProperties.sms(),
						transactionProcessConfigurationProperties.smsStartStatus(), "booked",
						// TODO: SET
						// eventDetails
						"", null)))
...
//      Try to send SMS
		snsService
				.send(digitalCourtesySmsRequest.getReceiverDigitalAddress(), digitalCourtesySmsRequest.getMessageText())

//                The SMS in sent, publish to Notification Tracker with next status -> SENT
				.flatMap(publishResponse -> {
					generatedMessageDto
							.set(new GeneratedMessageDto().id(publishResponse.messageId()).system("systemPlaceholder"));
					return sqsService.send(notificationTrackerSqsName.statoSmsName(),
							new NotificationTrackerQueueDto(requestId, clientId, now(),
									...
									smsPresaInCaricoInfo.getStatusAfterStart(), "sent",
...
//                 An error occurred during SMS send, start retries
				.onErrorResume(SnsSendException.class, snsSendException -> retrySmsSend(acknowledgment,
						smsPresaInCaricoInfo, smsPresaInCaricoInfo.getStatusAfterStart(), generatedMessageDto.get()))
...
//                TODO: CHANGE THE PAYLOAD
	...
	private Mono<SendMessageResponse> retrySmsSend(final Acknowledgment acknowledgment,
			final SmsPresaInCaricoInfo smsPresaInCaricoInfo, final String currentStatus,
...
//                       The SMS in sent, publish to Notification Tracker with next status -> SENT
				.flatMap(publishResponse -> sqsService.send(notificationTrackerSqsName.statoSmsName(),
						new NotificationTrackerQueueDto(requestId, clientId, now(),
								transactionProcessConfigurationProperties.sms(), currentStatus, "sent",
								// TODO: SET eventDetails
								"",
...
//                       The maximum number of retries has ended
	...
	private Mono<SendMessageResponse> smsRetriesExceeded(final Acknowledgment acknowledgment,
			final SmsPresaInCaricoInfo smsPresaInCaricoInfo, final GeneratedMessageDto generatedMessageDto,
...
//      Publish to Notification Tracker with next status -> RETRY
		return sqsService
				.send(notificationTrackerSqsName.statoSmsName(),
						new NotificationTrackerQueueDto(requestId, clientId, now(),
								transactionProcessConfigurationProperties.sms(), currentStatus, "retry",
								// TODO: SET eventDetails
								"", generatedMessageDto))
