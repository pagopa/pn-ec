...
import it.pagopa.pn.ec.rest.v1.dto.CourtesyMessageProgressEvent.EventCodeEnum;
		...
		return insertRequestFromSms(digitalCourtesySmsRequest)
				.then(sqsService.send(notificationTrackerSqsName.statoSmsName(),
						new NotificationTrackerQueueDto(presaInCaricoInfo.getRequestIdx(),
								presaInCaricoInfo.getXPagopaExtchCxId(), now(), INVIO_SMS, null, BOOKED.getValue(),
								// TODO: Populate
								// GeneratedMessageDto
								// Use this syntax
								// new
...
//                                                            Publish to SMS INTERACTIVE or SMS BATCH
				.flatMap(sendMessageResponse -> {
					DigitalCourtesySmsRequest.QosEnum qos = smsPresaInCaricoInfo.getDigitalCourtesySmsRequest()
							.getQos();
					if (qos == INTERACTIVE) {
						return sqsService.send(smsSqsQueueName.interactiveName(), smsPresaInCaricoInfo);
					} else if (qos == BATCH) {
	...
	private Mono<RequestDto> insertRequestFromSms(final DigitalCourtesySmsRequest digitalCourtesySmsRequest) {
		return Mono.fromCallable(() -> {
			...
			digitalRequestPersonalDto
					.setQos(DigitalRequestPersonalDto.QosEnum.valueOf(digitalCourtesySmsRequest.getQos().name()));
...
//                           The SMS in sent, publish to Notification Tracker with next status -> SENT
				.flatMap(publishResponse -> sqsService.send(notificationTrackerSqsName.statoSmsName(),
						new NotificationTrackerQueueDto(requestId, clientId, now(), INVIO_SMS,
								currentRequestStatus.get(), SENT.getValue(),
								// TODO: Populate
								// GeneratedMessageDto
								// Use this syntax new
...
//                           TODO: CHANGE THE PAYLOAD
	...
	private Mono<SendMessageResponse> retrySmsSend(final SmsPresaInCaricoInfo smsPresaInCaricoInfo,
...
//                       The SMS in sent, publish to Notification Tracker with next status -> SENT
				.then(sqsService.send(notificationTrackerSqsName.statoSmsName(),
						new NotificationTrackerQueueDto(requestId, clientId, now(), INVIO_SMS, RETRY.getValue(),
								SENT.getValue(),
								// TODO: Populate GeneratedMessageDto
...
//                                  Publish to Notification Tracker with next status -> ERROR
				.flatMap(requestDto -> sqsService.send(notificationTrackerSqsName.statoSmsName(),
						new NotificationTrackerQueueDto(requestId, clientId, now(), INVIO_SMS,
								requestDto.getStatusRequest(), ERROR.getValue(),
								// TODO: Populate
								// GeneratedMessageDto
								// Use this syntax new
								// GeneratedMessageDto().id
...
//                                  Publish to ERRORI SMS queue
	...
	public Flux<CourtesyMessageProgressEvent> getCourtesyShortMessageStatus(String requestIdx,
			String xPagopaExtchCxId) {

		return authService.clientAuth(xPagopaExtchCxId)
				.flatMap(unused -> gestoreRepositoryCall.getRichiesta(requestIdx)).doOnNext(requestDTO -> {

					List<CourtesyMessageProgressEvent> eventsList = new ArrayList<CourtesyMessageProgressEvent>();
					...
					for (EventsDto eventDTO : eventsListDTO) {
						...
						// !!Da risolvere conflitto status cliente e status interni!!
						// event.setStatus(ProgressEventCategory.valueOf(eventsDTO.getStatus().name()));
						event.setStatus(null);

						// !!Da risolvere conflitto event code cliente e event code interni!!
						// event.setEventCode(EventCodeEnum.valueOf(eventsDTO.getEventCode().name()));
						event.setEventCode(null);

						var digMessageRef = new DigitalMessageReference();
						var generatedMessageDTO = digProgrStatus.getGeneratedMessage();
						digMessageRef.setId(generatedMessageDTO.getId());
						digMessageRef.setLocation(generatedMessageDTO.getLocation());
						digMessageRef.setSystem(generatedMessageDTO.getSystem());
