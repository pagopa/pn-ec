    ...
    public Mono<RequestMetadata> updateEventsMetadata(String requestIdx, Events events) {
        return Mono.fromCompletionStage(requestMetadataDynamoDbTable.getItem(getKey(requestIdx)))
                   ...
                   .flatMap(retrieveRequestMetadata -> {
                       ...
                       if (events.getDigProgrStatus() != null) {
                           ...
                           if (eventsList != null) {
                               ...
                               for (Events eve : eventsList) {
                                   ...
                                   if (events.getDigProgrStatus().getEventTimestamp().isBefore(eve.getDigProgrStatus().getEventTimestamp()))
                                       generalStatus = null;
                           ...
                           if (eventsList != null) {
                               ...
                               for (Events eve : eventsList) {
                                   if (events.getPaperProgrStatus()
                                             .getStatusDateTime()
                                             .isBefore(eve.getPaperProgrStatus().getStatusDateTime())) generalStatus = null;
                       ...
                       return callMacchinaStati.statusDecode(processID, statusToConvert, clientID).map(macchinaStatiDecodeResponseDto -> {

                                                                                                           if (events.getDigProgrStatus() != null) {
                                                                                                               events.getDigProgrStatus().setEventCode(macchinaStatiDecodeResponseDto.getLogicStatus());
                                                                                                           } else {
                                                                                                               events.getPaperProgrStatus().setStatusCode(macchinaStatiDecodeResponseDto.getLogicStatus());
                                                                                                           }
