...
//      Initialize paperRequestDto
	...
	private static Stream<Arguments> provideDigitalAndPaperRequestToInsert() {
		return Stream.of(Arguments.of(digitalRequest, "idDigitalToInsert"),
				Arguments.of(paperRequest, "idPaperToInsert"));
	}
	...
	void insertRequestTestSuccess(RequestDto requestDto, String newId) {
		...
		webClient.post().uri(gestoreRepositoryEndpointProperties.postRequest()).accept(APPLICATION_JSON)
				.contentType(APPLICATION_JSON).body(BodyInserters.fromValue(requestDto)).exchange().expectStatus()
				.isOk();
	}
	...
	void insertRequestTestFailed() {
		webClient.post().uri(gestoreRepositoryEndpointProperties.postRequest()).accept(APPLICATION_JSON)
				.contentType(APPLICATION_JSON).body(BodyInserters.fromValue(digitalRequest)).exchange().expectStatus()
	...
	void readRequestTestSuccess(String id) {
		webClient.get().uri(uriBuilder -> uriBuilder.path(gestoreRepositoryEndpointProperties.getRequest()).build(id))
				.accept(APPLICATION_JSON).exchange().expectStatus().isOk().expectBody(RequestDto.class);
	}
	...
	void readRequestTestFailed() {
		webClient.get().uri(
				uriBuilder -> uriBuilder.path(gestoreRepositoryEndpointProperties.getRequest()).build("idNotExist"))
				.accept(APPLICATION_JSON).exchange().expectStatus().isNotFound();
	}
	...
	private static Stream<Arguments> provideDigitalAndPaperEventToUpdate() {
		...
		return Stream.of(Arguments.of(digitalEventDto, DEFAULT_ID_DIGITAL),
				Arguments.of(paperEventDto, DEFAULT_ID_PAPER));
	}
	...
	void testUpdateSuccess(EventsDto eventsDto, String idRequest) {
		...
		webClient.patch()
				...
				.accept(APPLICATION_JSON).contentType(APPLICATION_JSON).body(BodyInserters.fromValue(eventsDto))
	...
	void testUpdateFailed(EventsDto eventsDto) {
		...
		webClient.patch()
				.uri(uriBuilder -> uriBuilder.path(gestoreRepositoryEndpointProperties.patchRequest())
						.build("idCheNonEsiste"))
				.accept(APPLICATION_JSON).contentType(APPLICATION_JSON).body(BodyInserters.fromValue(eventsDto))
				.exchange().expectStatus().isNotFound();
	}

	private static Stream<Arguments> provideDigitalAndPaperRequestForDelete() {
		return Stream.of(Arguments.of(digitalRequest, "idDigitalToDelete"),
				Arguments.of(paperRequest, "idPaperToDelete"));
	}
	...
	void deleteRequestTestSuccess(RequestDto requestDto, String idToDelete) {
		...
		insertRequestPersonal(idToDelete,
				objectMapper.convertValue(requestDto.getRequestPersonal(), RequestPersonal.class));
	...
	void deleteRequestTestFailed() {
		webClient.delete().uri(uriBuilder -> uriBuilder.path(gestoreRepositoryEndpointProperties.deleteRequest())
				.build("idCheNonEsiste")).accept(APPLICATION_JSON).exchange().expectStatus().isNotFound();
	}
}
